...
CREATE TABLE IF NOT EXISTS ops_event (
  ...
  old_state          ENUM('AWAITING_CONTRACT','AVAILABLE','PICKED_UP','DELIVERED','FAILED','EXPIRED') NULL,
  new_state          ENUM('AWAITING_CONTRACT','AVAILABLE','PICKED_UP','DELIVERED','FAILED','EXPIRED') NOT NULL,
  acceptor_name      VARCHAR(255) NULL,
  created_at         TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS discord_config (
  ...
  auto_thread_create_on_request TINYINT(1) NOT NULL DEFAULT 0,
  thread_auto_archive_minutes INT NOT NULL DEFAULT 1440,
...
CREATE TABLE IF NOT EXISTS discord_thread (
  ...
  ops_channel_id VARCHAR(64) NOT NULL,
  anchor_message_id VARCHAR(64) NULL,
  thread_state ENUM('active','archived','missing','creating') NOT NULL DEFAULT 'active',
  thread_created_at DATETIME NULL,
  thread_last_posted_at DATETIME NULL,
  created_at  DATETIME NOT NULL DEFAULT UTC_TIMESTAMP(),
  updated_at  DATETIME NOT NULL DEFAULT UTC_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (request_id),
  UNIQUE KEY uq_discord_thread_id (thread_id),
...
CREATE TABLE IF NOT EXISTS dnf_rule (
  ...
  apply_pickup       TINYINT(1) NOT NULL DEFAULT 1,
  apply_delivery     TINYINT(1) NOT NULL DEFAULT 1,
  apply_transit      TINYINT(1) NOT NULL DEFAULT 1,
  reason             VARCHAR(255) NULL,
